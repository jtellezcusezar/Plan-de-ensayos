...
def _mix(c1, c2, ratio):
    ...
    p = max(0.0, min(1.0, float(ratio)))
    return _rgb_to_hex((r1 + (r2 - r1) * p, g1 + (g2 - g1) * p, b1 + (b2 - b1) * p))
...
primary = st.get_option("theme.primaryColor") or "#4A7BA8"
bg = st.get_option("theme.backgroundColor") or "#F6F8FB"
surface = st.get_option("theme.secondaryBackgroundColor") or "#FFFFFF"
text = st.get_option("theme.textColor") or "#111827"
dark_mode = _is_dark(bg)

chart_colors = st.get_option("theme.chartCategoricalColors") or []

def _chart_color(idx, fallback):
    return chart_colors[idx] if idx < len(chart_colors) else fallback


COLORS = {
    "Planeado": _chart_color(0, "#7BA7D4"),
    "Completo": _chart_color(1, "#6BBF9E"),
    "Incompleto": _chart_color(2, "#E8C17A"),
    "No Realizado": _chart_color(3, "#D98B8B"),
}

THEME = {
    "bg": bg,
    "surface": surface,
    "surface_alt": _mix(surface, bg, 0.22 if dark_mode else 0.12),
    "text": text,
    "muted": _mix(text, bg, 0.45 if dark_mode else 0.58),
    "border": _mix(text, bg, 0.78 if dark_mode else 0.90),
    "grid": _mix(text, bg, 0.84 if dark_mode else 0.92),
    "hover_bg": _mix(surface, bg, 0.18 if dark_mode else 0.08),
    "accent": primary,
    "accent_soft": _mix(primary, surface, 0.80 if dark_mode else 0.86),
    "accent_border": _mix(primary, surface, 0.68 if dark_mode else 0.76),
    "input_bg": _mix(surface, bg, 0.14 if dark_mode else 0.06),
    "input_text": text,
    "focus_shadow": _rgba(primary, 0.18 if dark_mode else 0.12),
    "plotly_hover_bg": _mix(text, bg, 0.10 if dark_mode else 0.90),
    "plotly_hover_text": bg if not dark_mode else text,
    "plotly_hover_border": _mix(text, bg, 0.55 if dark_mode else 0.45),
    "scrollbar": _mix(text, bg, 0.70 if dark_mode else 0.82),
    "header_shadow": _rgba("#000000", 0.45 if dark_mode else 0.07),
    "card_shadow": _rgba("#000000", 0.35 if dark_mode else 0.05),
    "card_shadow_hover": _rgba("#000000", 0.50 if dark_mode else 0.08),
    "hm_100_bg": _mix(COLORS["Completo"], surface, 0.72 if dark_mode else 0.45),
    "hm_100_text": _mix(COLORS["Completo"], text, 0.25 if dark_mode else 0.60),
    "hm_75_bg": _mix(COLORS["Incompleto"], surface, 0.76 if dark_mode else 0.58),
    "hm_75_text": _mix(COLORS["Incompleto"], text, 0.25 if dark_mode else 0.62),
    "hm_50_bg": _mix(COLORS["Incompleto"], surface, 0.82 if dark_mode else 0.68),
    "hm_50_text": _mix(COLORS["Incompleto"], text, 0.32 if dark_mode else 0.70),
    "hm_25_bg": _mix(COLORS["No Realizado"], surface, 0.82 if dark_mode else 0.68),
    "hm_25_text": _mix(COLORS["No Realizado"], text, 0.32 if dark_mode else 0.70),
    "hm_0_bg": _mix(COLORS["No Realizado"], surface, 0.70 if dark_mode else 0.46),
    "hm_0_text": _mix(COLORS["No Realizado"], text, 0.22 if dark_mode else 0.60),
    "hm_na_bg": _mix(surface, bg, 0.22 if dark_mode else 0.10),
    "hm_na_text": _mix(text, bg, 0.60 if dark_mode else 0.76),
    "badge_complete_bg": _mix(COLORS["Completo"], surface, 0.84 if dark_mode else 0.80),
    "badge_complete_text": _mix(COLORS["Completo"], text, 0.35 if dark_mode else 0.60),
    "badge_incomplete_bg": _mix(COLORS["Incompleto"], surface, 0.84 if dark_mode else 0.80),
    "badge_incomplete_text": _mix(COLORS["Incompleto"], text, 0.35 if dark_mode else 0.60),
    "badge_none_bg": _mix(COLORS["No Realizado"], surface, 0.84 if dark_mode else 0.80),
    "badge_none_text": _mix(COLORS["No Realizado"], text, 0.35 if dark_mode else 0.60),
    "badge_plan_bg": _mix(COLORS["Planeado"], surface, 0.84 if dark_mode else 0.80),
    "badge_plan_text": _mix(COLORS["Planeado"], text, 0.35 if dark_mode else 0.60),
    "ok_bg": _mix(COLORS["Completo"], surface, 0.86 if dark_mode else 0.80),
    "ok_border": _mix(COLORS["Completo"], surface, 0.72 if dark_mode else 0.62),
    "ok_text": _mix(COLORS["Completo"], text, 0.35 if dark_mode else 0.62),
}
...
.app-header{{background:var(--surface);border-bottom:1px solid var(--border);padding:14px 32px;display:flex;align-items:center;justify-content:space-between;margin:-1rem -2rem 0 -2rem;position:sticky;top:0;z-index:100;box-shadow:0 1px 6px var(--header-shadow);}}
.logo-box{{width:36px;height:36px;background:linear-gradient(135deg,{COLORS["Planeado"]},{THEME["accent"]});border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px;}}
...
div[data-testid="stSelectbox"]>label{{font-size:11px!important;font-weight:600!important;color:var(--muted)!important;text-transform:uppercase!important;letter-spacing:.05em!important;margin-bottom:4px!important;}}
div[data-testid="stSelectbox"]>div>div{{border-radius:10px!important;border:1.5px solid var(--border)!important;background:var(--input-bg)!important;font-size:13px!important;color:var(--input-text)!important;}}
div[data-testid="stSelectbox"]>div>div:focus-within{{border-color:var(--accent)!important;box-shadow:0 0 0 3px var(--focus-shadow)!important;}}
div[data-testid="stTextInput"]>label{{font-size:11px!important;font-weight:600!important;color:var(--muted)!important;text-transform:uppercase!important;letter-spacing:.05em!important;}}
div[data-testid="stTextInput"]>div>input{{border-radius:10px!important;border:1.5px solid var(--border)!important;background:var(--input-bg)!important;font-size:13px!important;color:var(--input-text)!important;}}
div[data-testid="stTextInput"]>div>input:focus{{border-color:var(--accent)!important;box-shadow:0 0 0 3px var(--focus-shadow)!important;}}
.kpi-card{{background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:18px 20px;position:relative;overflow:hidden;box-shadow:0 1px 4px var(--card-shadow);transition:transform .15s,box-shadow .15s;}}
.kpi-card:hover{{transform:translateY(-2px);box-shadow:0 6px 20px var(--card-shadow-hover);}}
.kpi-card::before{{content:'';position:absolute;top:0;left:0;right:0;height:3px;border-radius:14px 14px 0 0;}}
.kp-blue::before{{background:{COLORS["Planeado"]};}}.kp-green::before{{background:{COLORS["Completo"]};}}.kp-yellow::before{{background:{COLORS["Incompleto"]};}}.kp-red::before{{background:{COLORS["No Realizado"]};}}.kp-slate::before{{background:linear-gradient(90deg,{COLORS["Planeado"]},{COLORS["Completo"]});}}
...
.kp-blue .kpi-value{{color:{COLORS["Planeado"]};}}.kp-green .kpi-value{{color:{COLORS["Completo"]};}}.kp-yellow .kpi-value{{color:{COLORS["Incompleto"]};}}.kp-red .kpi-value{{color:{COLORS["No Realizado"]};}}.kp-slate .kpi-value{{color:var(--accent);}}
...
.sem-card{{background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:14px 18px;min-width:160px;position:relative;overflow:hidden;box-shadow:0 1px 4px var(--card-shadow);transition:transform .15s;}}
...
.sv::after{{background:{COLORS["Completo"]};}}.sa::after{{background:{COLORS["Incompleto"]};}}.sr::after{{background:{COLORS["No Realizado"]};}}
...
.sv .sem-tasa{{color:{COLORS["Completo"]};}}.sa .sem-tasa{{color:{COLORS["Incompleto"]};}}.sr .sem-tasa{{color:{COLORS["No Realizado"]};}}
...
with tab1:
    ...
    fill_map_line = {
        "Completo": _rgba(COLORS["Completo"], 0.15 if not dark_mode else 0.20),
        "Incompleto": None,
        "No Realizado": None,
    }
...
with tab2:
    ...
    if not df2.empty:
        ...
        fig_tasa.add_vline(x=META, line_dash="dot", line_color=THEME["accent"], line_width=1.5,
...
with tab3:
    ...
    if not adf.empty:
        ...
        fig_area = go.Figure()
        fill_map_area = {
            "Completo": _rgba(COLORS["Completo"], 0.15 if not dark_mode else 0.22),
            "Incompleto": _rgba(COLORS["Incompleto"], 0.12 if not dark_mode else 0.20),
            "No Realizado": _rgba(COLORS["No Realizado"], 0.10 if not dark_mode else 0.18),
        }
    ...
    if not crit_df.empty:
        ...
        st.markdown(
            f'<div style="overflow-x:auto;border-radius:10px;border:1px solid {THEME["border"]};">'
...
with tab4:
    ...
    if not df4.empty:
        ...
        rows4 = "".join(
            ...
            f"<td style='max-width:180px;white-space:normal;font-size:11px;color:{THEME['muted']}'>{r.Frecuencia}</td>"
        ...
        st.markdown(
            f'<div style="overflow-x:auto;border-radius:10px;border:1px solid {THEME["border"]};max-height:480px;overflow-y:auto;">'
            f'<table class="rt"><thead><tr><th>Proyecto</th><th>Etapa</th><th>Material</th>'
